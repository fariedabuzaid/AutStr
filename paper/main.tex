...
$d = 1$ the ring code paths reproduce the field constructions
byte-for-byte. Two engineering variants are worth recording: the
implemented protocol realises the merges of \autoref{thm:master} in two
stages with a seventh register buffering the sibling's claim, and the
advice of the fixed-center classes can be emitted either as one letter
per position or in the streamed one-entry-per-letter form of
\autoref{thm:master}, which is what makes widths $r \ge 2$ over the
ring representable in practice.
